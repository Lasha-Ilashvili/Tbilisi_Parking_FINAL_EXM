...
class ActiveLicensesListAdapter :
    ListAdapter<ActiveLicense, ActiveLicensesListAdapter.ActiveLicensesViewHolder>(ActiveLicensesDiffUtil) {
        ...
        fun bind() {
            ...
            with(binding) {
                ivLicenseBackground.apply {
                    background.setTint(Color.parseColor(activeLicense.license.licenseType.backgroundColor))
                    background.alpha = 100

                    setColorFilter(Color.parseColor(activeLicense.license.licenseType.backgroundColor))
                }

                tvPeriod.text = itemView.context.getString(activeLicense.license.validity.toInt())
                tvLicenseType.text = itemView.context.getString(activeLicense.license.licenseType.type)
                tvPrice.text = activeLicense.license.price.toString()
                tvStatus.text = activeLicense.status
