...
import com.example.tbilisi_parking_final_exm.domain.usecase.edit_vehicle.EditVehicleUseCase
...
class EditVehicleViewModel @Inject constructor(
    ...
    private fun editVehicle(vehicleId: Int, name: String){
        viewModelScope.launch {
            editVehicleUseCase(EditVehicle(carId = vehicleId, name = name).toDomain()).collect{
                when (it) {
                    is Resource.Success -> _uiEvent.emit(EditVehicleUiEvent.NavigateToParkingFragment)
